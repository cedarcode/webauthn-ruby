  ...
  module AttestationStatement
    class TPM < Base
      class PubArea
        ...
        def valid?(public_key, alg)
          cose_key = COSE::Key.deserialize(public_key)

          case cose_key
          when COSE::Key::EC2
            valid_ecc_key?(cose_key, alg)
          when COSE::Key::RSA
            valid_rsa_key?(cose_key, alg)
          else
        ...
        def valid_ecc_key?(cose_key, alg)
          t_public.parameters.symmetric == ::TPM::ALG_NULL &&
            (t_public.parameters.scheme == ::TPM::ALG_NULL || t_public.parameters.scheme == COSE_TO_TPM_ALG[alg]) &&
            t_public.parameters.curve_id == COSE_TO_TPM_CURVE[cose_key.crv] &&
            t_public.unique.buffer == cose_key.x + cose_key.y
        end

        def valid_rsa_key?(cose_key, alg)
          t_public.parameters.symmetric == ::TPM::ALG_NULL &&
            (t_public.parameters.scheme == ::TPM::ALG_NULL || t_public.parameters.scheme == COSE_TO_TPM_ALG[alg]) &&
